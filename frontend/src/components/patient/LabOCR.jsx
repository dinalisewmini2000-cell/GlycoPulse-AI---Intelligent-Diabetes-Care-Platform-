...
import { 
  ...
  Sparkles, RefreshCw, AlertCircle, FileCheck, ArrowUpRight, RotateCcw 
} from 'lucide-react';
      ...
      <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center', border: '2px dashed var(--accent-cyan)', background: 'rgba(6, 182, 212, 0.03)' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', alignItems: 'center' }}>
          <button onClick={() => fileInputRef.current?.click()} className="btn-glow">
            <UploadCloud size={16} />
            <span>{uploadedFile ? 'Select Different File' : 'Select File to Parse'}</span>
          </button>
          
          {(uploadedFile || ocrResults) && (
            <button onClick={handleResetOcr} className="btn-outline" style={{ borderColor: 'var(--accent-cyan)' }}>
              <RotateCcw size={16} />
              <span>Refresh & Clear Report</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div className="glass-panel" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem', flexWrap: 'wrap', gap: '0.8rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
              <button onClick={handleResetOcr} className="btn-outline" style={{ fontSize: '0.78rem', padding: '0.3rem 0.7rem' }}>
                <RotateCcw size={14} />
                <span>Parse Another Report</span>
              </button>
