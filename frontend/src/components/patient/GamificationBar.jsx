...
export const GamificationBar = () => {
  ...
  const badges = [
    { title: '7-Day TIR Master', desc: 'Maintain ≥70% Time-in-Range for 7 consecutive days', unlocked: false, icon: Trophy, color: '#f59e0b' },
    { title: 'Night Sentinel', desc: 'Zero nocturnal hypoglycemia events for 14 nights', unlocked: false, icon: ShieldCheck, color: '#06b6d4' },
    { title: 'Fiber Champion', desc: 'Log 30g+ dietary fiber 5 days in a row', unlocked: false, icon: Star, color: '#10b981' },
    { title: 'Hydration Hero', desc: 'Meet 2.5L daily hydration goal 10 days in a row', unlocked: false, icon: Target, color: '#a855f7' }
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '0.5rem' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.82rem', color: 'var(--accent-amber)', fontWeight: 600 }}>
              Level {level} Novice Member | XP: {xp} / {nextLevelXp}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.2rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '1.25rem', borderLeft: '4px solid var(--accent-amber)' }}>
          ...
          <div style={{ fontSize: '2.2rem', fontWeight: 900, color: 'var(--accent-amber)' }}>{streakDays || 1} Day</div>
        ...
        <div className="glass-panel" style={{ padding: '1.25rem', borderLeft: '4px solid var(--accent-cyan)' }}>
          ...
          <div style={{ fontSize: '2.2rem', fontWeight 900, color: 'var(--accent-cyan)' }}>{healthScore || 100}/100</div>
          <span style={{ fontSize: '0.75rem', color: 'var(--accent-cyan)' }}>Initial Baseline</span>
        </div>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem' }}>
        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem' }}>Clinical Badges</h3>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.2rem' }}>
          {badges.map((b, idx) => {
            const Icon = b.icon;
            return (
              <div key={idx} style={{ background: 'var(--bg-secondary)', padding: '1.25rem', borderRadius: '12px', border: 'var(--border-color)', borderLeft: `4px solid ${b.color}`, opacity: b.unlocked ? 1 : 0.75 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                  ...
                  <div>
                    ...
                    <span className={`badge ${b.unlocked ? 'badge-success' : 'badge-warning'}`}>{b.unlocked ? 'Unlocked' : 'Locked'}</span>
                  </div>
                </div>
